...
from _paths import JEDI_CACHE_DIR  # noqa: E402
...
def test_find_references():
    ...
    file_path = fr.module_to_file(TOY_PKG_DIR, "toy_pkg", "toy_pkg.b")
    assert file_path == TOY_PKG_DIR / "b.py"

    line, col = fr.find_definition_position(file_path, "helper")
    assert (line, col) == (1, 4), (line, col)

    project = jedi.Project(str(TOY_PKG_DIR.parent))
    script = jedi.Script(path=str(file_path), project=project)
    refs = script.get_references(line=line, column=col)
    call_sites = sorted(
        (Path(r.module_path).name, r.line) for r in refs if not r.is_definition()
    )
    assert call_sites == [("a.py", 5), ("e.py", 5)], call_sites
...
def test_module_dotted_for_file():
    assert br.module_dotted_for_file(TOY_PKG_DIR, "toy_pkg", "b.py") == "toy_pkg.b"
    assert br.module_dotted_for_file(TOY_PKG_DIR, "toy_pkg", "__init__.py") == "toy_pkg"
