...
class LiveTest(unittest.TestCase):
 def test_real_codeql(self):
  ...
  enriched={"codeql":{"diagnostic":"not started"}};decision=None
  try:
   ...
   payload={"decision":decision.to_dict() if decision else None,"codeql":enriched.get("codeql"),"semanticEdges":enriched.get("semanticEdges",[])};DIAGNOSTIC.write_text(json.dumps(payload,indent=2,default=str),encoding="utf-8")
