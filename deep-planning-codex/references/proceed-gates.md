...
- Do not present a final execution gate while `adversarial-review.md` has a `FAIL` or `PARTIAL` verdict or any `BLOCKING` finding.
...
When `$handoff-codex` is available, direct it to write `.deep-planning/handoffs/<gate>.md` for this workflow. If unavailable, write the same concise handoff locally.
