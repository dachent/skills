...
FULL_VALIDATION_PREFIXES = (".github/actions/", ".generated/")
...
def affected_skill_names(
    ...
    if changed & FULL_VALIDATION_PATHS or any(
        any(matches_path(path, prefix) for prefix in FULL_VALIDATION_PREFIXES)
        for path in changed
    ):
        return sorted(names)
    affected: set[str] = set()
    for skill in skills:
        if any(matches_path(path, skill["path"]) for path in changed):
            affected.add(skill["name"])
        for value in skill.get("packaging", {}).values():
            if isinstance(value, str) and any(matches_path(path, value) for path in changed):
                affected.add(skill["name"])
    for component in manifest.get("shared_components", []):
        component_path = component.get("path")
        if isinstance(component_path, str) and any(matches_path(path, component_path) for path in changed):
    ...
    for mirror in manifest.get("generated_mirrors", []):
        ...
        if any(
            ...
            and any(matches_path(path, candidate) for path in changed)
            for candidate in (source, destination)
        ):
            for skill in skills:
                if isinstance(source, str) and matches_path(source, skill["path"]):
                    affected.add(skill["name"])
...
def specialized_skill_names(
    ...
    if full:
        selected = set(affected_skill_names(manifest, changed_files, full=True))
        return sorted(selected & SPECIALIZED_WORKFLOWS.keys())
    changed = {normalize_path(path) for path in changed_files if path.strip()}
    ...
    for name, workflow in SPECIALIZED_WORKFLOWS.items():
        ...
        if skill is None:
            continue
        if any(matches_path(path, skill["path"]) for path in changed) or normalize_path(workflow) in changed:
            selected.add(name)
...
def runners_for_skill(skill: dict[str, Any]) -> list[str]:
    ...
    if unknown:
        raise ValueError(f"{skill.get('name', '<unknown>')}: unsupported CI platforms: {', '.join(unknown)}")
    if not runners:
...
def build_matrix(manifest: dict[str, Any], selected_names: Iterable[str]) -> dict[str, list[dict[str, Any]]]:
    by_name = skill_map(manifest)
...
def build_plan(
    ...
    selected = affected_skill_names(manifest, changed, full=full)
    specialized = specialized_skill_names(manifest, changed, full=full)
    ...
    return {
        ...
        "specialized_workflows": {name: SPECIALIZED_WORKFLOWS[name] for name in specialized},
    }
