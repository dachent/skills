...
| Calculation state | Rebuild calculation when formulas, external links, PivotTables, Power Query, or cached values matter. | Cached values can be stale even when formulas look correct in OOXML. | Run `scripts/refresh_excel.ps1` from an Excel COM-capable session. |
...
Normal Codex execution can inspect OOXML structure, edit formulas, build cleaned tables, write Power Query M text files, create static workbooks, and run `scripts/check_formula_errors.ps1` on supported OOXML files.
